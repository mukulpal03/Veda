...
import { useAssessmentProgress, useAssessmentResults, useDocumentUpload } from './index';
...
export function useExtractionProcess() {
  ...
  const startPipeline = useCallback(async () => {
    ...
    const progressTimer = setInterval(() => {
      if (activeStage < 3) {
        ...
        updateProgress(EXTRACTION_STAGES[activeStage].label, EXTRACTION_STAGES[activeStage].targetPercent);
      }
    }, 1400);
    ...
    try {
      ...
      if (response && response.ok) {
        ...
        if (realAssessmentData && realAssessmentData.questions && realAssessmentData.questions.length > 0) {
  ...
  }, [router, updateProgress, setStatus, setResults, questionPaperPages, answerSheetPages, questionPaperTexts, answerSheetTexts]);
